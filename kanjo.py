...
╚═════╝      ╚═╝      ╚═════╝
    ...
    y, x = stdscr.getmaxyx()
    lines = logo.split("\n")
    start_y = max(0, (y - len(lines)) // 2)
    start_x = max(0, (x - max(len(line) for line in lines)) // 2)
...
def display_user_info(stdscr, email, password, balance, status):
    ...
    stdscr.addstr(17, 2, f"Password: {'*' * len(password)}", curses.color_pair(color))
    stdscr.addstr(18, 2, f"User: Multiple BTC", curses.color_pair(color))
